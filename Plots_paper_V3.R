...
nreads_totais_uteis_summary<-data.frame(region = c('ITS','V1V2','V2V3','V3V4','V4V5','V5V7','V7V9'),
                                   total=c(14275,750924,801072,668509,1674525,881612,749534),
                                   after_filtering=c(NA,614973,624939,488560,477043,766525,634974))
...
F1A<-ggplot(reshape2::melt(nreads_totais_uteis_summary), aes(x=region,y=value,fill=region,color=variable))+
  geom_bar_pattern(stat = "identity", 
                   pattern = c("stripe", "stripe", "stripe", "stripe","stripe","stripe","stripe", # 1st col
                               "none", "none", "none", "none", "none", "none","none" # 2nd col
                   ),
                   pattern_angle = c(rep(45, 7), rep(0, 7)),
                   pattern_density = .1,
                   pattern_spacing = .05,
                   pattern_fill = 'white',
                   aes(fill = region),
  ...
  scale_fill_manual(values = c("gray55", brewer.pal(6, "Dark2")))+
...
observed_genera$taxa<-rownames(observed_genera)
observed_genera_melt<-reshape2::melt(observed_genera,"taxa")
observed_genera_melt$value<-as.factor(observed_genera_melt$value)

FS4B<-ggplot(reshape2::melt(observed_genera,"taxa"), aes(y=variable,x=taxa))+geom_tile(aes(fill=as.factor(value)),color="black",size=1)+
...
observed_phyla$taxa<-rownames(observed_phyla)
observed_phyla_melt<-reshape2::melt(observed_phyla,"taxa")
observed_phyla_melt$value<-as.factor(observed_phyla_melt$value)

FS4A<-ggplot(reshape2::melt(observed_phyla,"taxa"), aes(y=variable,x=taxa))+geom_tile(aes(fill=as.factor(value)),color="black",size=1)+
  scale_fill_manual(values=c("lightyellow","firebrick3"),labels=c("0"="Undetected","1"="Detected"))+
